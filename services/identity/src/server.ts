...
const mailer=smtpConfigured?nodemailer.createTransport({host:process.env.SMTP_HOST,port:Number(process.env.SMTP_PORT??465),secure:(process.env.SMTP_SECURE??"true")==="true",auth:{user:process.env.SMTP_USER,pass:process.env.SMTP_PASS}}):null;
async function sendResetEmail(email:string,token:string){if(!mailer)return false;const appUrl=process.env.APP_URL??"http://localhost:5173";await mailer.sendMail({from:process.env.MAIL_FROM??`NexaPlay <${process.env.SMTP_USER}>`,to:email,subject:"Mã đặt lại mật khẩu NexaPlay",text:`Mã đặt lại mật khẩu của bạn: ${token}\n\nMã có hiệu lực 30 phút. Mở ${appUrl}, chọn Đăng nhập > Quên mật khẩu > Tôi đã có mã đặt lại.\n\nNếu bạn không yêu cầu, hãy bỏ qua email này.`,html:`<h2>Đặt lại mật khẩu NexaPlay</h2><p>Mã của bạn:</p><p style="font:700 18px monospace;word-break:break-all">${token}</p><p>Mã có hiệu lực trong 30 phút.</p><p>Mở <a href="${appUrl}">${appUrl}</a>, chọn <b>Đăng nhập → Quên mật khẩu → Tôi đã có mã đặt lại</b>.</p><p>Nếu bạn không yêu cầu, hãy bỏ qua email này.</p>`});return true}
...
app.post("/v1/auth/forgot-password",asyncHandler(async(req,res)=>{const parsed=z.object({email:z.string().email()}).safeParse(req.body);if(!parsed.success)return fail(res,400,"VALIDATION_ERROR","Email không hợp lệ");const user=(await pool.query("SELECT id FROM users WHERE email=$1 AND status='active'",[parsed.data.email.toLowerCase()])).rows[0];let delivered=false;if(user&&mailer){const raw=opaqueToken();await pool.query("UPDATE password_reset_tokens SET used_at=NOW() WHERE user_id=$1 AND used_at IS NULL",[user.id]);await pool.query("INSERT INTO password_reset_tokens(id,user_id,token_hash,expires_at) VALUES($1,$2,$3,NOW()+INTERVAL '30 minutes')",[`prt_${nanoid(16)}`,user.id,tokenHash(raw)]);try{delivered=await sendResetEmail(parsed.data.email,raw)}catch(error){console.error("reset email delivery failed",error)}}const emailMode=smtpConfigured;ok(res,{accepted:true,delivery:emailMode?"email":"manual",message:emailMode?(delivered?"Nếu email tồn tại, mã đặt lại đã được gửi. Vui lòng kiểm tra cả thư rác.":"Nếu email tồn tại, hệ thống sẽ gửi mã đặt lại. Vui lòng kiểm tra cả thư rác."):"Email gửi tự động chưa được cấu hình. Hãy liên hệ quản trị viên để nhận mã sau khi xác minh tài khoản."})}));
